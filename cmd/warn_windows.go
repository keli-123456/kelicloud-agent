...
// ShowToast 在用户态中执行
func ShowToast() {
	title := "kelicloud Agent is Running"
	message := "The remote control software \"kelicloud Agent\" is running, which allows others to control your computer. If this was not initiated by you, please terminate the program immediately."

	const aumid = "kelicloud.Agent"
	const linkName = "kelicloud Warning (Auto Delete Later)"
...
func ensureStartMenuShortcut(aumid, linkName string) error {
	...
	_, _ = oleutil.PutProperty(shortcut, "Description", "kelicloud Agent")
	// 设置 AUMID
